...
$q_baru = mysqli_query($conn, "SELECT COUNT(*) as total FROM permohonan WHERE keterangan='Baru'");
$total_baru = mysqli_fetch_assoc($q_baru)['total'];

$q_ganti = mysqli_query($conn, "SELECT COUNT(*) as total FROM permohonan WHERE keterangan='Penggantian'");
$total_ganti = mysqli_fetch_assoc($q_ganti)['total'];

// Mengambil data untuk Grafik Jenis Kelamin
$q_jk = mysqli_query($conn, "SELECT jenis_kelamin, COUNT(*) as jumlah FROM permohonan GROUP BY jenis_kelamin");
$label_jk = []; $data_jk = [];
while($row = mysqli_fetch_assoc($q_jk)) {
    $label_jk[] = ($row['jenis_kelamin'] == 'L') ? 'Laki-laki' : 'Perempuan';
    $data_jk[]  = $row['jumlah'];
}

// Mengambil data 5 permohonan terbaru untuk mini-tabel
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card:nth-child(2)::before { background: var(--imigrasi-gold); }
        .stat-card:nth-child(3)::before { background: var(--text-blue-grey); }
...
<body>
            ...
            <div class="card-container" style="padding: 0;">
                ...
                <table style="margin-top: 16px;">
                    ...
                    <tbody>
                        ...
                        ?>
                        <tr>
                            <td style="padding-left: 28px;"><?php echo htmlspecialchars($row['nama_pemohon']); ?></td>
                        ...
                        <?php 
                            ...
                            echo "<tr><td colspan='4' style='text-align: center; padding: 30px; color: var(--text-blue-grey);'>Belum ada data tersedia.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
    ...
    <script>
        ...
        const dataBaru = <?php echo $total_baru; ?>;
        const dataGanti = <?php echo $total_ganti; ?>;

        // Skema Warna: Imigrasi Navy, Gold, Light Blue, Blue-Grey
        const colors = ['#0B427B', '#F5A623', '#475569', '#E2E8F0'];
